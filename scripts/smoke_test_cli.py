...
just "did it crash"):
  ...
  4. REMEDIATOR's #14/#15 path-containment fix is exercised against a REAL
     binary path (the `sleep` coreutils binary, resolved via
     /proc/<pid>/exe), not a mock -- and correctly refuses to generate a
     rollback script for it, since `sleep` reliably lives under a
     protected system directory (/bin or /usr/bin) on every mainstream
     Linux distribution.
...
def main() -> int:
    ...
    try:
        ...
        # Use `sleep`, not sys.executable -- see the identical fix and full
        # reasoning in tests/integration/test_chain_sentinel_router_harness.py
        # (a Python interpreter's install path varies wildly -- venv, pyenv,
        # actions/setup-python's /opt/hostedtoolcache/, a system package
        # under /usr -- while `sleep` reliably resolves under /bin or
        # /usr/bin on every mainstream Linux distribution, which is what
        # makes assertion 4 below actually portable).
        ...
        try:
            ...
            if "ABORTED" in action_types and any(
                ...
                print(
                    ...
                    "real sleep-binary path, and #14/#15's containment fix correctly refused it "
                    "(protected system directory)."
                )
            else:
                failures.append(
                    "Expected REMEDIATOR to dispatch and refuse the real sleep-binary path "
