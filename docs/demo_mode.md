...
REVERSER     ABORTED                  Refusing remediation on critical system path: /usr/bin/sleep (inside /usr)...
...
- **`REVERSER ABORTED` is expected, not a failure.** In this trace the
  "malware" is a real `sleep` (coreutils) process (the harmless subprocess
  technique itself, chosen deliberately for its predictable install
  location), so `/proc/<pid>/exe` resolves to the real `sleep` binary --
  which correctly lives under a protected system directory (`/bin` or
  `/usr/bin`, depending on distro). Remediator's [`known_issues.md`](known_issues.md) #15
